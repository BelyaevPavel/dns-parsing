    ...
    def handle(self, *args, **options):
        for url in options['URL']:
            try:
                ...
                hreference_list=['www.dns-shop.ru'+e.get('href') for e in root.cssselect("div.product div.item-name a.ec-price-item-link")]
                ...
                for i in range(len(product_name_list)):
                    p=Product(product_name = product_name_list[i],hreference = hreference_list[i],product_price = product_price_list[i].replace(' ',''))
                    p.save()
                    self.stdout.write(self.style.SUCCESS('i = %i name = %s ref = %s price = %s' % (i,p.product_name, p.hreference, p.product_price)))
